...
with st.sidebar:
    st.header("⚙️ 系統核心設定")
    ticker_input = st.text_input("🚀 輸入股票代碼 (多個請用逗號隔開):", value="0050",
                                 placeholder="例如: 2330, 0050, TSLA")
    ...
    for full_ticker in search_list:
        try:
            ...
            if not df.empty and len(df) > 20:
                ...
                actual_last = info.get('regularMarketPrice') if info.get('regularMarketPrice') else float(
                    df['Close'].iloc[-1])
                actual_prev = info.get('regularMarketPreviousClose') if info.get(
                    'regularMarketPreviousClose') else float(df['Close'].iloc[-2])
...
if not tickers:
    ...
    for ticker in tickers:
        ...
        if result:
            ...
            col1.metric("當前收盤價", f"{last_price:.2f}", f"{day_change:+.2f} ({day_pct:+.2f}%)",
                        delta_color="inverse")
            col2.metric("今日成交區間 (高/低)", f"{df['High'].iloc[-1]:.2f}", f"{df['Low'].iloc[-1]:.2f}",
                        delta_color="off")
            ...
            with t1:
                ...
                fig.add_trace(go.Scatter(x=df['Date'], y=df['Upper'], name='上軌',
                                         line=dict(dash='dash', color='rgba(150,150,150,0.3)')))
                fig.add_trace(go.Scatter(x=df['Date'], y=df['Lower'], name='下軌',
                                         line=dict(dash='dash', color='rgba(150,150,150,0.3)'), fill='tonexty'))
